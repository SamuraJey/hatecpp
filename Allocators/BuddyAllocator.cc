...
char* BuddyAllocator::allocate(size_t size) {
    ...
    size_t req_level;
    /*
    В этом фрагменте кода используется ассемблерная инструкиця для эффективного определения наименьшего уровня в иерархии списков, необходимого для выделения запрошенного размера памяти.
    Команда "bsr" (Bit Scan Reverse) находит индекс наибольшего значащего бита в аргументе "allocateSize - 1 << 1", что позволяет определить требуемый уровень,
    на котором должен находиться блок памяти для выделения запрошенного размера. Это решение позволяет избежать дорогостоящих вычислений и быстро получить необходимый уровень.
    Зачем констркуция `allocateSize - 1 << 1`?
    Это делается для определения наименьшего уровня в иерархии списков, необходимого для выделения запрошенного размера памяти.
    `allocateSize - 1 << 1` - это выражение используется для определения требуемого уровня.
    ...
    */
    ...
    // немного побитовой магии и цикла как ни бывало
